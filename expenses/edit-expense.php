...
// Access check
check_access('expenses', 'edit');

$message = '';
$id = isset($_GET['id']) ? mysqli_real_escape_string($connection, trim($_GET['id'])) : '';
...
// Fetch target expense record
$fetch_sql = "SELECT * FROM tbl_expenses WHERE id = '$id' AND deleted_at IS NULL LIMIT 1";
$fetch_res = mysqli_query($connection, $fetch_sql);
if (!$fetch_res || mysqli_num_rows($fetch_res) === 0) {
    header('Location: expenses-list.php?msg=error');
    exit;
}

$expense = mysqli_fetch_assoc($fetch_res);
    ...
    if ($is_nozzle_expense && $nozzle_id <= 0) {
        ...
        $sql = "UPDATE tbl_expenses 
                SET expense_date = '$expense_date', 
                    ...
                    nozzle_id = $nozzle_sql_val, 
                    amount = '$amount', 
                    payment_method = '$payment_method', 
                    bank_id = $bank_sql_val, 
        ...
        if (mysqli_query($connection, $sql)) {
            header('Location: expenses-list.php?msg=updated');
            exit;
        } else {
            $message = '<div class="alert alert-danger"><i class="fas fa-exclamation-triangle mr-1"></i> Error updating expense: ' . mysqli_error($connection) . '</div>';
        }
    } else {
...
// Fetch active nozzles (including currently attached nozzle even if deactivated)
$current_nozzle_id = intval($expense['nozzle_id'] ?? 0);
$nozzles_res = mysqli_query($connection, "
    ...
    WHERE (n.status = 'Active' OR n.id = '$current_nozzle_id') 
    ...
    <main class="main">
        <div class="container pt-4 pb-4">
            ...
            <div class="row mb-3 align-items-center">
                <div class="col-md-6">
                    <h4><i class="fas fa-edit mr-2" style="color: var(--primary-color);"></i>Edit Expense Record</h4>
                </div>
            ...
            <div class="card shadow-sm">
                <div class="card-header card-header-navy font-weight-bold">
                    <i class="fas fa-receipt mr-2"></i>Edit Expense #<?php echo $expense['id']; ?>
                </div>
                <div class="card-body">
                    <form method="POST" action="">
                        <div class="row">
                            ...
                            <div class="col-md-4 form-group">
                                ...
                                <select name="expense_type_id" id="expense_type_id" class="form-control" required onchange="toggleNozzleField()">
                                    ...
                                    <?php if ($types_res && mysqli_num_rows($types_res) > 0): ?>
                                        ...
                                        ?>
                                            <option value="<?php echo $t['id']; ?>" data-is-nozzle="<?php echo $isNozCat ? '1' : '0'; ?>" <?php echo ($expense['expense_type_id'] == $t['id']) ? 'selected' : ''; ?>>
                            ...
                            <div class="col-md-4 form-group">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="number" step="0.01" min="0.01" name="amount" class="form-control" value="<?php echo htmlspecialchars($expense['amount']); ?>" required>
                                </div>
                            </div>
                        </div>
                                ...
                                <select name="nozzle_id" id="nozzle_id" class="form-control">
                                    ...
                                    <?php if ($nozzles_res && mysqli_num_rows($nozzles_res) > 0): ?>
                                        ...
                                        <?php while ($noz = mysqli_fetch_assoc($nozzles_res)): ?>
                                            <option value="<?php echo $noz['id']; ?>" <?php echo (intval($expense['nozzle_id'] ?? 0) === intval($noz['id'])) ? 'selected' : ''; ?>>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4 form-group" id="bankGroup" style="display: <?php echo ($expense['payment_method'] !== 'Cash') ? 'block' : 'none'; ?>;">
                            ...
                            <div class="col-md-4 form-group">
                                ...
                                <input type="text" name="reference_no" class="form-control" value="<?php echo htmlspecialchars($expense['reference_no'] ?? ''); ?>">
                            </div>
                        </div>
