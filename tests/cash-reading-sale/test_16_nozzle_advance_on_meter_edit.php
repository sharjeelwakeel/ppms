...
$test_date = '2029-01-16';
$test_shift = 1;
$nozzle_id = 1;

test_header('TC-NOZ-02', 'Running Counter Advance on Latest Meter Edit');

try {
    reset_test_date_data($connection, $test_date);

    // Initial reading: 1,500.00
    ...
    // Operator edits closing reading to 1,750.00
    mysqli_query($connection, "UPDATE tbl_meter_reading_details SET current_reading = 1750.00 WHERE meter_reading_id = '$mr_id' AND nozzle_id = '$nozzle_id'");

    // Simulate edit-meter-reading.php sync logic
    ...
    if ($latest_mr_check && $row = mysqli_fetch_assoc($latest_mr_check)) {
        if (intval($row['id']) === $mr_id) {
            mysqli_query($connection, "UPDATE tbl_nozzles SET start_reading = 1750.00 WHERE id = '$nozzle_id'");
        }
    }
    ...
    assert_eq(floatval($noz['start_reading'] ?? 0), 1750.00, "tbl_nozzles.start_reading must update to edited meter (1750.00)");
