...
$test_date = '2029-01-07';
$test_shift = 1;
$nozzle_id = 1;

test_header('TC-CASH-07', 'Card POS Sale Fuel Deduction from Cash Remainder');
...
try {
    ...
    VALUES ('$test_date', '$test_shift', '$nozzle_id', 1, 200.00, 200.00, 40000.00, 800.00, 39200.00)");
    ...
    assert_eq(floatval($cash['quantity'] ?? 0), 800.00, "Cash quantity must reduce to 1000 - 200 = 800.00 Ltr");
    assert_eq(floatval($cash['amount'] ?? 0), 160000.00, "Cash amount must reduce to 800 * 200 = Rs. 160,000.00");

} finally {
    reset_test_date_data($connection, $test_date);
}

exit(print_suite_summary('TC-CASH-07'));
