<?php
require_once __DIR__ . '/../config.php';
require_once __DIR__ . '/../test_helper.php';
require_once 'd:/xampp/htdocs/ppms/include/cash_automation_helper.php';

$test_date = '2029-01-10';
$test_shift = 1;
$nozzle_id = 1;

test_header('TC-CASH-10', 'Zero-Floor Clamping Invariant');

try {
    reset_test_date_data($connection, $test_date);

    // 1. Meter: 500 Ltr
    mysqli_query($connection, "INSERT INTO tbl_meter_readings (date, shift_id, payment_type, grand_total) VALUES ('$test_date', '$test_shift', 'Cash', 100000.00)");
    $mr_id = mysqli_insert_id($connection);
    mysqli_query($connection, "INSERT INTO tbl_meter_reading_details (meter_reading_id, nozzle_id, last_reading, current_reading, sale_reading, test_reading, net_sale, price, amount) 
    VALUES ('$mr_id', '$nozzle_id', 0.00, 500.00, 500.00, 0.00, 500.00, 200.00, 100000.00)");

    // 2. Mistakenly add Credit Slip of 600 Ltr (> 500 Ltr)
    mysqli_query($connection, "INSERT INTO tbl_meter_reading_credit_sales 
    (slip_no, slip_date, sale_date, shift_id, nozzle_id, account_number, vehicle_number, quantity, issue_quantity, rate, amount)
    VALUES ('CR-TEST-10', '$test_date', '$test_date', '$test_shift', '$nozzle_id', 1, 'ABC-123', 600.00, 600.00, 200.00, 120000.00)");

    sync_shift_cash_sales($connection, $test_date, $test_shift);

    $q = mysqli_query($connection, "SELECT * FROM tbl_meter_reading_cash_sales WHERE sale_date = '$test_date' AND shift_id = '$test_shift' AND nozzle_id = '$nozzle_id'");
    $cash = mysqli_fetch_assoc($q);

    assert_eq(floatval($cash['quantity'] ?? -1), 0.00, "Cash quantity must clamp to 0.00 Ltr (never negative)");
    assert_eq(floatval($cash['amount'] ?? -1), 0.00, "Cash amount must clamp to Rs. 0.00 (never negative)");
    assert_true(strpos($cash['notes'] ?? '', 'exceeded Meter') !== false, "Notes must contain warning note explaining non-cash exceeded meter");

} finally {
    reset_test_date_data($connection, $test_date);
}

exit(print_suite_summary('TC-CASH-10'));
