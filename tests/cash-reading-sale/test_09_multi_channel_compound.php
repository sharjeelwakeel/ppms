...
$test_date = '2029-01-09';
$test_shift = 1;
$nozzle_id = 1;

test_header('TC-CASH-09', 'Multi-Channel Compound Settlement Reconciliation');
...
try {
    ...
    VALUES ('CR-TEST-09', '$test_date', '$test_date', '$test_shift', '$nozzle_id', 1, 'ABC-123', 400.00, 400.00, 200.00, 80000.00)");

    // 3. Add Card = 350 Ltr
    mysqli_query($connection, "INSERT INTO tbl_meter_reading_card_sales 
    (sale_date, shift_id, nozzle_id, card_machine_id, quantity, rate, amount, service_charges, net_amount)
    VALUES ('$test_date', '$test_shift', '$nozzle_id', 1, 350.00, 200.00, 70000.00, 1400.00, 68600.00)");
    ...
    $cash_qty = floatval($cash['quantity'] ?? 0);
    $cash_amt = floatval($cash['amount'] ?? 0);

    assert_eq($cash_qty, 250.00, "Cash quantity must equal 1000 - 400 - 350 = 250.00 Ltr");
    assert_eq($cash_amt, 50000.00, "Cash amount must equal 250 * 200 = Rs. 50,000.00");
    assert_eq($cash_qty + 400.00 + 350.00, 1000.00, "Settled volume (Cash 250 + Credit 400 + Card 350) must equal 1,000.00 Ltr exactly");

} finally {
    reset_test_date_data($connection, $test_date);
}

exit(print_suite_summary('TC-CASH-09'));
