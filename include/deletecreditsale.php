...
if (isset($_POST['date']) && !empty($_POST['date'])) {
    ...
    $q_prev = mysqli_query($connection, "SELECT nozzle_id, shift_id, SUM(quantity) AS total_qty 
                                          ...
                                          WHERE slip_date = '$date' $shift_clause AND (deleted_at IS NULL OR deleted_at = '0000-00-00 00:00:00') 
    ...
    $id = intval($_POST['id']);
    // Revert dispensed petrol litres for single credit slip
    $q_single = mysqli_query($connection, "SELECT nozzle_id, quantity, slip_date, shift_id FROM tbl_meter_reading_credit_sales WHERE id = '$id' AND (deleted_at IS NULL OR deleted_at = '0000-00-00 00:00:00')");
    if ($q_single && $s_row = mysqli_fetch_assoc($q_single)) {
        $s_noz   = intval($s_row['nozzle_id']);
        $s_qty   = floatval($s_row['quantity']);
        $s_date  = $s_row['slip_date'];
