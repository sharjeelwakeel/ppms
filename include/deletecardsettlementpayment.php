...
try {
    ...
    $q_pay = mysqli_query($connection, "SELECT id, total_amount, payment_date, card_machine_id, bank_id 
                                        FROM tbl_card_settlement_payments 
                                        WHERE id = '$paymentId' 
                                          AND (deleted_at IS NULL OR deleted_at = '0000-00-00 00:00:00') 
                                        FOR UPDATE");
    if (!$q_pay || mysqli_num_rows($q_pay) == 0) {
        throw new Exception("Deposit voucher not found or already deleted.");
    }
    ...
    $q_alloc = mysqli_query($connection, "SELECT id, settlement_id, allocated_amount 
                                          FROM tbl_card_settlement_payment_allocations 
    ...
    if ($q_alloc) {
        while ($alloc = mysqli_fetch_assoc($q_alloc)) {
            ...
            // Revert paid_amount and payment_status on the settlement batch
            $q_batch = mysqli_query($connection, "SELECT net_amount, paid_amount 
                                                  FROM tbl_card_sale_settlements 
                                                  WHERE id = '$settlementId' 
                                                  FOR UPDATE");
            if ($q_batch && ($batch = mysqli_fetch_assoc($q_batch))) {
                $oldPaid   = floatval($batch['paid_amount']);
                $netAmt    = floatval($batch['net_amount']);
    ...
    mysqli_query($connection, "UPDATE tbl_card_settlement_payment_allocations 
                               SET deleted_at = NOW() 
                               WHERE payment_id = '$paymentId' 
                                 AND (deleted_at IS NULL OR deleted_at = '0000-00-00 00:00:00')");

    // 4. Soft delete master payment voucher
    mysqli_query($connection, "UPDATE tbl_card_settlement_payments 
                               SET deleted_at = NOW() 
                               WHERE id = '$paymentId'");
    ...
    echo json_encode([
        'status'  => 'success',
        'message' => 'Deposit voucher #<strong>' . $paymentId . '</strong> (Rs. ' . number_format($payment['total_amount'], 2) . ') has been soft deleted and settlement balances have been rolled back successfully.'
    ]);
