...
// Enforce access check for adding customers
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name)) {
        $message = '<div class="alert alert-danger"><i class="fas fa-exclamation-circle mr-1"></i> Please provide a valid customer name.</div>';
    } else {
        $query = "INSERT INTO tbl_customers (name, phone, address, fuel_rate, other_rate, status) 
                  VALUES ('$name', '$phone', '$address', '$fuel_rate', '$other_rate', '$status')";
        if (mysqli_query($connection, $query)) {
            header('Location: customers-list.php?msg=added');
            exit;
        } else {
            $message = '<div class="alert alert-danger"><i class="fas fa-exclamation-circle mr-1"></i> Error registering customer: ' . mysqli_error($connection) . '</div>';
        }
    }
}
?>
            ...
            <div class="row mb-3 align-items-center">
                <div class="col-md-6">
                    <h4 class="font-weight-bold" style="color:var(--primary-color);">
                        <i class="fas fa-user-plus mr-2 text-primary"></i>Add New Customer
                    </h4>
                    <p class="text-muted small mb-0">Register a customer profile with default pricing rates and account status.</p>
                </div>
                    ...
                    <form action="add-customer.php" method="POST">
                        ...
                        <div class="row">
                            <div class="col-md-6 form-group">
                                ...
                                <input type="text" name="name" class="form-control" placeholder="e.g. Al-Madina Transport / Ali Ahmed" required>
                            </div>
                            <div class="col-md-6 form-group">
                        ...
                        <div class="row">
                            <div class="col-12 form-group">
                                ...
                                <textarea name="address" class="form-control" rows="2" placeholder="e.g. Plot # 45, Industrial Area, Karachi"></textarea>
                            </div>
                        </div>
