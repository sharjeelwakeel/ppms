...
// Self-healing migration for auxiliary columns in tbl_card_sale_settlements
...
// Fetch active card machines with fee & revenue charge
$card_machines = [];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($settlement_date)) {
        ...
        $sql = "INSERT INTO tbl_card_sale_settlements 
                (card_machine_id, settlement_date, shift_id, batch_no, no_of_cards, amount, charges_percentage, service_charges, revenue_percentage, revenue_amount, net_amount, notes)
                VALUES 
                ('$card_machine_id', '$settlement_date', '$shift_id', '$batch_no', '$no_of_cards', '$total_amount', '$charges_pct', '$service_charges', '$revenue_pct', '$revenue_amount', '$net_amount', '$notes')";
...
<div class="container-fluid mt-4 px-3 px-lg-4 mb-5" style="max-width: 960px;">
    <div class="page-header">
        <div>
            <h4><i class="fas fa-file-invoice-dollar mr-2 text-warning"></i> Add Card Sale Settlement</h4>
            <small class="text-white-50">Record bank POS terminal batch settlement and cross-verify with system readings</small>
        </div>
    ...
    <form method="POST" id="addSettlementForm" onsubmit="return validateSettlementForm(event)">
        <!-- Form Details Card -->
        <div class="form-card mb-4">
            <div class="form-card-header d-flex justify-content-between align-items-center">
                <span><i class="fas fa-info-circle mr-2"></i> Settlement & Batch Identification</span>
                <span class="badge badge-light text-primary font-weight-bold" id="badgeMatchStatus">Waiting for input</span>
            </div>
            <div class="p-4">
                <div class="row">
                    <!-- Settlement Date -->
                    <div class="col-md-6 col-sm-6 mb-3">
                        ...
                        <input type="date" name="settlement_date" id="settlement_date" class="form-control font-weight-bold" value="<?php echo date('Y-m-d'); ?>" required onchange="onDateChange()">
                    </div>
                    ...
                    <div class="col-md-6 col-sm-6 mb-3">
                        ...
                        <input type="text" name="batch_no" id="batch_no" list="batch_no_list" class="form-control font-weight-bold" placeholder="Enter Batch No from POS Slip..." required oninput="onBatchInputChange()" onchange="onBatchInputChange()" autocomplete="off">
                ...
                <div class="p-3 bg-light border rounded d-flex flex-wrap justify-content-between align-items-center mt-2">
                    <div>
                        ...
                        <small class="text-muted" id="lblPopupSummaryDesc">Select Machine and enter Batch No to preview calculation breakdown in popup.</small>
                    </div>
            ...
            <div class="p-4">
                <div class="row">
                    ...
                    <div class="col-md-4 col-sm-6 mb-3">
                        ...
                        <input type="number" step="0.01" min="0.01" name="total_amount" id="total_amount" class="form-control font-weight-bold text-primary" placeholder="Pure Sales (Gross)..." required oninput="onPureAmountInputChange()">
                    ...
                    <div class="col-md-4 col-sm-12 mb-3">
                        ...
                        <input type="number" step="0.01" min="0.01" name="net_amount" id="amount" class="form-control font-weight-bold text-success" placeholder="Net deposit from POS slip..." required>
                        <small class="text-muted">Settled deposit amount from POS terminal slip</small>
                    </div>

                    <!-- Hidden Auxiliary Calculations -->
                    <input type="hidden" name="charges_percentage" id="charges_percentage" value="0.0000">
                    <input type="hidden" name="service_charges" id="service_charges" value="0.00">
                    <input type="hidden" name="revenue_percentage" id="revenue_percentage" value="0.0000">
                    <input type="hidden" name="revenue_amount" id="revenue_amount" value="0.00">
                    ...
                    <div class="col-md-12 mb-3">
                        ...
                        <textarea name="notes" id="notes" class="form-control" rows="2" placeholder="Optional notes regarding this terminal batch settlement..."></textarea>
                    </div>
                </div>
                                ...
                                <tbody id="batchItemsBody">
                                    <tr>
                                        <td colspan="6" class="text-muted text-center py-3">
                                            <i class="fas fa-search mr-1"></i> Select Card Machine and enter Batch No to load itemized transactions.
                                        </td>
                                    </tr>
                                </tbody>
        ...
        if (Math.abs(diff) > 0.01) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    document.getElementById('addSettlementForm').submit();
                } else {
                    $('#amount').focus();
                }
            });
