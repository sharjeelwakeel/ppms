		...
		<main class="main">
			<div class="container pt-4 pb-4">
				...
				<table id="shiftsListTable" class="table table-striped table-bordered">
					...
					<tbody>
						<?php 
						$sql = "SELECT * FROM tbl_shifts ORDER BY id DESC";
						...
						if($resultcheck > 0){
							while($row = mysqli_fetch_assoc($result)){
								...
								echo' 
									<tr>
										<td>'.$row['id'].'</td>
										<td><a href="edit-shift.php?id='.$row['id'].'" class="font-weight-bold" style="color: var(--primary-color);">'.htmlspecialchars($row['name']).'</a></td>
										<td>'.date("h:i A", strtotime($row['start_time'])).'</td>
										<td>'.date("h:i A", strtotime($row['end_time'])).'</td>
										<td><span class="badge '.$statusBadge.'">'.htmlspecialchars($row['status']).'</span></td>
										<td><a class="btn btn-large btn-link p-0 text-danger" onclick="deleteshift('.$row['id'].')"><i class="fas fa-trash-alt" style="font-size: 20px;"></i></a></td>
									</tr>';
							}
						}
						?>
