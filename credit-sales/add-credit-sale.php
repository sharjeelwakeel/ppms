        ...
        if ($valid) {
            ...
            try {
                ...
                foreach ($unique_nozzles as $un_id) {
                    recalculate_meter_reading_from_sales($connection, $sale_date, $shift_id, $un_id, "Credit Sales entry");
                }
...
function addCreditRowWithData(data) {
    ...
    if (data) {
        ...
        if (parseFloat(data.rate || 0) > 0) {
            $newRow.find('.credit-rate').val(parseFloat(data.rate).toFixed(2));
        } else {
...
function resolveCreditRowRate($row, callback) {
    ...
    $.getJSON('ajax-credit-slip-lookup.php', {
        ...
        if (res && res.status === 'success') {
            ...
            $row.find('.credit-cash-rate').val(ca > 0 ? ca.toFixed(2) : '');
        }
        calculateCreditRow($row.find('.credit-qty')[0]);
        if (callback) callback();
    }).fail(function() {
