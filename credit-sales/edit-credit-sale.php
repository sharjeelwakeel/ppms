        ...
        if ($valid) {
            ...
            try {
                ...
                for ($i = 0; $i < count($nozzles_arr); $i++) {
                    ...
                    $row_slip_date = !empty(trim($slip_dates_arr[$i] ?? '')) ? mysqli_real_escape_string($connection, trim($slip_dates_arr[$i])) : $new_date;
...
function addCreditRowWithData(data) {
    ...
    var rowHtml = '<tr id="credit_row_' + rowId + '">' +
        ...
        '</td>' +
        '<td>' +
            '<input type="date" name="credit_slip_date[]" class="form-control form-control-sm credit-slip-date font-weight-bold" value="' + slipDateVal + '">' +
        '</td>' +
        '<td>' +
