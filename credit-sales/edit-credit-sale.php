...
if (empty($target_date)) {
    header('Location: credit-sales-list.php');
    exit;
}
...
if (empty($existing_rows)) {
    // If no active rows, redirect to list
    header('Location: credit-sales-list.php');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($new_shift_id)) {
        ...
        $error_msg = 'Please keep at least one credit sale row.';
    } else {
        $valid = true;
            ...
            try {
                ...
                for ($i = 0; $i < count($nozzles_arr); $i++) {
                    ...
                    $ins_sql = "INSERT INTO tbl_meter_reading_credit_sales 
                                ...
                                VALUES 
                                (0, '$noz_id', '$new_date', '$new_shift_id', '$slip_no', '$slip_type', '$acc_num', '$veh_num',
...
<head>
    ...
    <title>PPMS - Edit Credit Sale Reading</title>
    <style>
...
<div class="container-fluid mt-4 px-3 px-lg-4 mb-5">
    <div class="page-header">
        <div>
            <h4><i class="fas fa-edit mr-2 text-warning"></i> Edit Credit Sale Reading</h4>
            <small class="text-white-50">Editing credit slips recorded for <?php echo date('d-m-Y', strtotime($target_date)); ?></small>
        </div>
            ...
            <div class="p-3">
                ...
                <div class="d-flex flex-wrap justify-content-between align-items-center mt-3 pt-3 border-top">
                    ...
                    <div class="d-flex flex-wrap align-items-center gap-3 mt-2 mt-md-0 summary-badge-box">
                        ...
                        <button type="submit" class="btn btn-primary font-weight-bold px-4 ml-md-3">
                            <i class="fas fa-save mr-1"></i> Update Credit Sales
                        </button>
                    </div>
                </div>
            </div>
...
var existingSlipsData = <?php echo json_encode($existing_rows); ?>;
var creditRowIdx = 0;

$(document).ready(function() {
    if (existingSlipsData && existingSlipsData.length > 0) {
        for (var i = 0; i < existingSlipsData.length; i++) {
            var row = existingSlipsData[i];
            addCreditRowWithData(row);
        }
    } else {
        addCreditRow();
    }
...
function addCreditRowWithData(data) {
    ...
    var isReturnedVal    = data ? parseInt(data.is_returned) : 0;
    ...
    var rowHtml = '<tr id="credit_row_' + rowId + '">' +
        ...
        '<td>' +
            '<div class="d-flex flex-column align-items-start">' +
                ...
                '</div>' +
                '<div class="mt-1 return-slip-box return-slip-box_' + rowId + '" style="' + (slipTypeVal === 'Temporary Slip' ? '' : 'display:none;') + '">' +
                    '<div class="custom-control custom-checkbox">' +
                        '<input type="hidden" name="credit_is_returned[]" class="credit-is-returned-val" value="' + isReturnedVal + '">' +
                        '<input type="checkbox" class="custom-control-input credit-is-returned" id="chk_ret_' + rowId + '" ' + (isReturnedVal === 1 ? 'checked' : '') + ' onchange="onReturnCheckboxChange(this)">' +
                    ...
                    '</div>' +
                    '<span class="badge ' + (isReturnedVal === 1 ? 'badge-success text-white' : 'badge-warning text-dark') + ' font-weight-bold p-1 mt-1 slip-return-status-text" style="font-size:9.5px; display:block; text-align:left;">' +
                        (isReturnedVal === 1 ? '<i class="fas fa-check-circle mr-1"></i> Received (Settled)' : '<i class="fas fa-hand-holding mr-1"></i> Giving (Loan Petrol)') +
                    '</span>' +
                '</div>' +
            '</div>' +
        ...
        '</td>' +
        '<td>' +
            '<input type="text" name="credit_slip_no[]" class="form-control form-control-sm credit-slip-no font-weight-bold text-monospace" placeholder="Slip #" value="' + slipNoVal + '" required>' +
        '</td>' +
        '<td>' +
            '<input type="text" name="credit_vehicle_number[]" list="registeredVehiclesList" class="form-control form-control-sm credit-vehicle-number font-weight-bold text-monospace" placeholder="Pick Vehicle" value="' + vehicleVal + '" oninput="onCreditVehicleInput(this)" onchange="onCreditVehicleInput(this)" required>' +
            '<div class="vehicle-match-info small text-left mt-1" style="display:none; font-size:10.5px; line-height:1.2;"></div>' +
        '</td>' +
        '<td><input type="text" name="credit_account_number[]" class="form-control form-control-sm credit-account-number font-weight-bold" placeholder="Cust ID" value="' + accountVal + '" readonly style="background-color:#e9ecef; cursor:not-allowed;" required></td>' +
...
function updateCreditItem(selectElement) {
    ...
    if (nz) {
        ...
        if (!$row.find('.credit-rate').val() || parseFloat($row.find('.credit-rate').val()) === 0) {
