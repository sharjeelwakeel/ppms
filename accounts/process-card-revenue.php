...
if (!has_permission('accounts', 'add') && !has_permission('accounts', 'edit') && !has_permission('card_sales', 'edit')) {
    echo json_encode(['status' => 'error', 'message' => 'Permission denied. You do not have access to collect card revenue charges.']);
    exit;
}
...
$paymentMode   = (isset($_POST['payment_mode']) && in_array($_POST['payment_mode'], ['Cash', 'Bank'])) ? $_POST['payment_mode'] : 'Cash';
...
$destinationLabel = 'Cash in Hand (Station Safe)';
if ($paymentMode === 'Bank') {
    if ($bankId <= 0) {
    ...
    $destinationLabel = $bankInfo['name'] . ' (' . $bankInfo['account_number'] . ')';
} else {
    $bankId = 0; // Cash mode
}

if ($amount <= 0.00) {
    echo json_encode(['status' => 'error', 'message' => 'Revenue collection amount must be greater than zero.']);
    exit;
}
...
try {
    // 1. Build open revenue settlement batches query with row locking (FOR UPDATE)
    $where_clauses = [
        "card_machine_id = '$cardMachineId'",
        "revenue_payment_status != 'Paid'",
        "(revenue_amount - revenue_paid_amount) > 0.00",
    ...
    $q_batches = "SELECT id, batch_no, settlement_date, revenue_amount, revenue_paid_amount 
    ...
    if (!$res_batches || mysqli_num_rows($res_batches) == 0) {
        $q_fallback = "SELECT id, batch_no, settlement_date, revenue_amount, revenue_paid_amount 
                       FROM tbl_card_sale_settlements 
    ...
    if (!$res_batches || mysqli_num_rows($res_batches) == 0) {
        ...
        echo json_encode(['status' => 'error', 'message' => 'No outstanding revenue charges found for this POS card machine.']);
        exit;
    }
    ...
    while ($b = mysqli_fetch_assoc($res_batches)) {
        $b_rev  = floatval($b['revenue_amount']);
        $b_paid = floatval($b['revenue_paid_amount']);
        $b_due  = max(0.00, round($b_rev - $b_paid, 2));
        if ($b_due > 0.00) {
            $b['due_amount'] = $b_due;
            $batches_to_settle[] = $b;
    ...
    if ($amount > $total_outstanding) {
        ...
        echo json_encode([
            'status' => 'error', 
            'message' => 'Revenue collection amount (Rs. ' . number_format($amount, 2) . ') cannot exceed the total outstanding revenue balance (Rs. ' . number_format($total_outstanding, 2) . ').'
        ]);
        exit;
    }

    // 2. Insert Master Revenue Receipt Voucher (tbl_card_revenue_payments)
    ...
    $bank_id_sql = ($bankId > 0) ? "'$bankId'" : "NULL";
    ...
    $ins_payment = "INSERT INTO tbl_card_revenue_payments 
                    (payment_date, card_machine_id, payment_mode, bank_id, total_amount, filter_from_date, filter_to_date, transaction_ref, remarks, created_by, created_at)
                    VALUES 
                    ($p_date_sql, '$cardMachineId', '$paymentMode', $bank_id_sql, '$amount', $f_from_sql, $f_to_sql, $trans_sql, $remarks_sql, '$userId', NOW())";

    if (!mysqli_query($connection, $ins_payment)) {
        throw new Exception("Error saving card revenue voucher: " . mysqli_error($connection));
    }
    ...
    foreach ($batches_to_settle as $batch) {
        ...
        $b_rev      = floatval($batch['revenue_amount']);
        $b_old_paid = floatval($batch['revenue_paid_amount']);
        ...
        if ($alloc > 0.00) {
            ...
            $ins_alloc = "INSERT INTO tbl_card_revenue_payment_allocations 
                          (payment_id, settlement_id, allocated_amount, created_at)
                          VALUES ('$paymentId', '$b_id', '$alloc', NOW())";
            if (!mysqli_query($connection, $ins_alloc)) {
                throw new Exception("Error recording revenue allocation for Batch #{$batch['batch_no']}: " . mysqli_error($connection));
            }

            // Update batch's revenue_paid_amount and revenue_payment_status
            $new_paid = round($b_old_paid + $alloc, 2);
            $new_status = ($new_paid >= $b_rev) ? 'Paid' : 'Partial';

            $upd_batch = "UPDATE tbl_card_sale_settlements 
                          SET revenue_paid_amount = '$new_paid', revenue_payment_status = '$new_status' 
                          WHERE id = '$b_id'";
            if (!mysqli_query($connection, $upd_batch)) {
                throw new Exception("Error updating revenue payment status for Batch #{$batch['batch_no']}: " . mysqli_error($connection));
            }
    ...
    $msg = "Card surcharge revenue of <strong>Rs. " . number_format($amount, 2) . "</strong> collected into <strong>" . htmlspecialchars($destinationLabel) . "</strong> successfully.<br>";
    if ($settled_count > 0) {
        $msg .= "• <strong>$settled_count</strong> batch(es) fully cleared for revenue.<br>";
    }
    if ($partial_count > 0) {
        $msg .= "• <strong>$partial_count</strong> batch(es) partially cleared.";
    }
