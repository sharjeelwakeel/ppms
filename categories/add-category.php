...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name)) {
        ...
        // Check duplicate
        $dupChk = mysqli_query($connection, "SELECT id FROM tbl_product_categories WHERE name = '$safeName' AND (deleted_at IS NULL OR deleted_at = '0000-00-00 00:00:00') LIMIT 1");
        if ($dupChk && mysqli_num_rows($dupChk) > 0) {
            $message = '<div class="alert alert-warning"><i class="fas fa-exclamation-triangle mr-1"></i> A category with this name already exists.</div>';
        } else {
            $insertSql = "INSERT INTO tbl_product_categories (name, description, status, created_at, updated_at) 
                          VALUES ('$safeName', '$safeDesc', '$safeStatus', NOW(), NOW())";
            if (mysqli_query($connection, $insertSql)) {
                header('Location: categories-list.php');
                exit;
            } else {
                $message = '<div class="alert alert-danger"><i class="fas fa-times-circle mr-1"></i> Error adding category: ' . mysqli_error($connection) . '</div>';
            }
        }
    }
}
...
<head>
    ...
    <title>PPMS - Add Product Category</title>
...
<main class="main">
    <div class="container py-4">
        ...
        <div class="page-header">
            <div>
                <h4><i class="fas fa-tags mr-2 text-warning"></i> Add Product Category</h4>
                <small class="text-white-50">Create a high-level classification for lubricant and retail products</small>
            </div>
            <div>
                    ...
                    <div class="row">
                        <div class="col-md-8">
                            <div class="form-group row">
                                ...
                                <div class="col-md-9">
                                    <input type="text" name="name" class="form-control" placeholder="e.g. Engine Oils, Brake Fluids, Greases" value="<?php echo htmlspecialchars($name); ?>" required autofocus>
                                    <small class="form-text text-muted">A distinct name identifying this product group.</small>
                                </div>
                            </div>
