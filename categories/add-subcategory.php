...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($category_id <= 0) {
        ...
        $dupChk = mysqli_query($connection, "SELECT id FROM tbl_product_subcategories 
                                             WHERE category_id = '$category_id' AND name = '$safeName' 
                                             AND (deleted_at IS NULL OR deleted_at = '0000-00-00 00:00:00') LIMIT 1");
        if ($dupChk && mysqli_num_rows($dupChk) > 0) {
            $message = '<div class="alert alert-warning"><i class="fas fa-exclamation-triangle mr-1"></i> A subcategory with this name already exists under the selected category.</div>';
        } else {
            $insertSql = "INSERT INTO tbl_product_subcategories (category_id, name, description, status, created_at, updated_at) 
                          VALUES ('$category_id', '$safeName', '$safeDesc', '$safeStatus', NOW(), NOW())";
            if (mysqli_query($connection, $insertSql)) {
                header('Location: subcategories-list.php?category_id=' . $category_id);
                exit;
            } else {
                $message = '<div class="alert alert-danger"><i class="fas fa-times-circle mr-1"></i> Error adding subcategory: ' . mysqli_error($connection) . '</div>';
            }
        }
    }
}
...
<head>
    ...
    <title>PPMS - Add Product Subcategory</title>
...
<main class="main">
    <div class="container py-4">
        ...
        <div class="page-header">
            <div>
                <h4><i class="fas fa-tag mr-2 text-warning"></i> Add Product Subcategory</h4>
                <small class="text-white-50">Create a sub-classification linked to a parent product category</small>
            </div>
            <div>
                <a href="subcategories-list.php<?php echo ($category_id > 0) ? '?category_id='.$category_id : ''; ?>" class="btn btn-light btn-sm font-weight-bold shadow-sm">
            ...
            <div class="data-card-body">
                <form action="add-subcategory.php" method="POST">
                    <div class="row">
                        <div class="col-md-8">
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-md-9">
                                    <input type="text" name="name" class="form-control" placeholder="e.g. 20W-50, Synthetic, DOT 4, Lithium Grease" value="<?php echo htmlspecialchars($name); ?>" required>
                                    <small class="form-text text-muted">A distinct sub-group name (e.g. viscosity grade, type, or packaging).</small>
                                </div>
                            </div>
                    ...
                    <div>
                        ...
                        <a href="subcategories-list.php<?php echo ($category_id > 0) ? '?category_id='.$category_id : ''; ?>" class="btn btn-secondary px-4 py-2 font-weight-bold ml-2">
