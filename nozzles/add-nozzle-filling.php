...
$message = '';
if (isset($_POST['nozzle_id']) && isset($_POST['tank_id']) && isset($_POST['last_reading']) && isset($_POST['current_reading'])) {
    ...
    $query = "INSERT INTO tbl_nozzle_filling (nozzle_id, tank_id, last_reading, current_reading) 
              VALUES ('$nozzle_id', '$tank_id', '$last_reading', '$current_reading')";
    
    if (mysqli_query($connection, $query)) {
        header('Location: nozzle-filling-list.php');
        exit;
    } else {
        $message = '<div class="alert alert-danger">Error saving nozzle filling record: ' . mysqli_error($connection) . '</div>';
    }
}
...
<html lang="en">
  <head>
    ...
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
		...
		<title>PPMS - Add Nozzle Filling</title>
	</head>
	<body>
		...
		<main class="main">
			<div class="container pt-4 pb-4">
				<form action="add-nozzle-filling.php" method="POST">
					<h4 class="mb-5">Add Nozzle Filling</h4>
                    <?php echo $message; ?>
					<div class="card mb-5">
						<div class="card-body">
							<div class="row">
								<div class="col-md-6">
											...
											<select name="nozzle_id" class="form-control" required>
                                                ...
                                                if (mysqli_num_rows($nozzles_result) > 0) {
                                                    while ($nozzle = mysqli_fetch_assoc($nozzles_result)) {
                                                        echo '<option value="' . $nozzle['id'] . '">' . htmlspecialchars($nozzle['name']) . '</option>';
                                                    }
                                                }
                                                ?>
											</select>
									...
									<div class="form-group row">
										...
										<div class="col-lg-9 col-md-7 col-sm-8">
											<input type="number" step="0.01" name="last_reading" class="form-control" placeholder="0.00" required>
										</div>
									</div>
									<div class="form-group row">
					...
					<div class="txt-center">
						<button type="submit" class="btn btn-primary m-top">Save Record</button>
